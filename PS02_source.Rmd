...
-  File > Save As
-  set the file name as `ps02_lastname_firstname` (example, `ps02_smith_john`)
...
Thes above packages are already installed on Appalachian's R Studio Server. **Every time** you open a new R session you need to **load (open)** any packages you want to use. Loading a package is done with the `library()` function.
...
<div id="answer">
R is considering `habit` to be of type `<chr>`, and `visits` is of type `<dbl>`. 
...
<div id="answer">
There's a positive relationship between `weight` and `weeks`.
...
ggplot(data = nc, aes(x = weeks, y = gained)) +
  geom_point() +
  labs(x = "Length of pregnancy (weeks)", 
       y = "Weight of mother (lbs)",
       title = "Relationship between pregnancy duration and mother's weight")
...
**A.** 
Adding the argument `color = premie` to the aes() function separated the premature birth and full term birth points on the graph by color.
     
**B.** 
There are now 3 variables displayed in this graph. 
     
**C.** 
36 weeks appears to be the cuttoff for classifying a newborn as a "premie" vs. a "full term".
...
ggplot(data = nc, aes(x = mage, y = weight, color = gender)) +
  ...
  labs(x = "Mother's age (years)",
       y = "Birth Weight (lbs)",
       title = "Mother's age vs. Birth weight (by gender)")
```

<div id="answer">
There does not seem to be a strong correllation between the mother's age and the birth weight of their newborns' as the values appear consistent with little exception. 
...
**A.** 
The y axis `count` represents the sum of observations that fall within each particular bin.

**B.** 
39 weeks appears to be the average length of pregnancies in this data set. 

**C.** 
If we were to change the binwidth to 100, there would then be just 1 bin in the histogram. I don't know why you'd use such a high value like that for this particular data set. 

</div> 

7.  Make a histogram of the birth `weight` of newborns (which is in lbs), include a descriptive title and axis labels.  Make the bins pink with a blue border. (code only to answer)

```{r}
ggplot(data = nc, aes(x = weight)) +
  geom_histogram(color = "blue", fill = "pink") +
  labs(x = "Birthweight of newborns (lbs)",
       y = "Count",
       title = "Weights of NC newborns at time of birth")
```
...
8.  Make a histogram of newborn birth `weight` split by `gender` of the child. Set the binwidth to 0.5. Which gender appears to have a slightly larger average birth weight? (code and text to answer)
...
# I wanted to have it so the sentence was in the format: The _(gender)_'s had a larger average.. but I found that there's whitespace after the genders in the data set so I did not feel like coding a whole other loop to remove it so I formatted the sentences a bit differently to make up for this. This is also why I ommitted the use of a period in the sentences.                                    
genderWithHighestWeight <- if(genderWeightSums$totalWeight[1] > genderWeightSums$totalWeight[2]) {
                            paste("The gender with the highest average birth weight according to the data set is ",                                                genderWeightSums$gender[1])
                           } else if(genderWeightSums$totalWeight[1] < genderWeightSums$totalWeight[2]){
                             paste("The gender with the highest average birth weight according to the data set is",                                                  genderWeightSums$gender[2])
                           } else if(genderWeightSums$totalWeight[1] == genderWeightSums$totalWeight[2]) {
                             paste("Neither gender had a larger average birth weight as they are currently the same in this data set")
                           } else {

                             paste("OOPS :( There's an issue with the data set! Use the contact form below referencing error code:                                  0x03Af16")
                           }
```

<div id="answer">
`r genderWithHighestWeight`
...
ggplot(data = nc, aes(x = mature, y = gained)) +
  geom_boxplot() +
  labs(x = "Maturity status",
       y = "Weight Gained",
       title = "Weight gained by pregnant mothers based on their maturity status")
```

<div id="answer">
`r paste("Based off my observation of the generated box plot, the younger mother's median weight gain during pregnancy than that of their more mature counterparts.")`



</div> 

10. Make a boxplot of pregnancy duration in `weeks` by smoking `habit`. Is the duration of pregnancy more **variable** for smokers or non-smokers? (i.e. which group has the greater spread for the variable `weeks`?). (code and text to answer)

```{r}
ggplot(data = nc, aes(x = habit, y = weeks)) +
  geom_boxplot() + 
  labs(x = "Smoker vs. Nonsmoker (Cigarettes)",
       y = "Preganancy Duration (Weeks)",
       title = "Smoking's effects on Pregnancy Duration")


```

<div id="answer">
`r paste("Telling from the above boxplot, the nonsomkers had the wider spread in terms of pregnancy duration.")`
...
<div id="answer">
The variance in the father's ages based on the nc data set is expansive and is therefore, assymetrical. 
...
<div id="answer">
According to this visualization, the median birth weight of babies is greater for white mothers than non-white mothers.
...
<div id="answer">
According to this visualization it appears the mother's age has much less to do with the duration of pregnancy than the number of pregnancies, as most of the observations are clustered around 9 months regardless of the mother's age but there are fewer data points the higher the mother's age.
